...
async function clean_build(callbackFunction: any) {
  const del = await import("del");

  del.deleteAsync(["build/behavior_packs/", "build/resource_packs/"]).then(
    (value) => {
...
async function clean_localmc(callbackFunction: any) {
  const del = await import("del");
...
async function clean_localmc_world(callbackFunction: any) {
  console.log("Removing '" + getTargetWorldPath() + "'");
  const del = await import("del");

  del
    .deleteAsync([getTargetWorldPath()], {
      force: true,
    })
    .then(
      (value) => {
...
async function clean_localmc_config(callbackFunction: any) {
  console.log("Removing '" + getTargetConfigPath() + "'");
  const del = await import("del");

  del
    .deleteAsync([getTargetConfigPath()], {
      force: true,
    })
    .then(
      (value) => {
...
async function clean_dev_world(callbackFunction: any) {
  console.log("Removing '" + getDevWorldPath() + "'");
  const del = await import("del");

  del
    .deleteAsync([getDevWorldPath()], {
      force: true,
    })
    .then(
      (value) => {
...
async function clean_localmc_world_backup(callbackFunction: any) {
  console.log("Removing backup'" + getTargetWorldBackupPath() + "'");
  const del = await import("del");

  del
    .deleteAsync([getTargetWorldBackupPath()], {
      force: true,
    })
    .then(
      (value) => {
...
async function clean_dev_world_backup(callbackFunction: any) {
...
const deploy_localmc = gulp.series(
  clean_localmc,
  function (callbackFunction: any) {
    if (!useMinecraftDedicatedServer) {
      console.log("\0o07"); // annunciate a beep!
    }
    callbackFunction();
  },
...
function stopServer(callbackFunction: any) {
  if (activeServer) {
...
function startServer(callbackFunction: any) {
  if (activeServer) {
    activeServer.stdin.write("stop\n");
    activeServer = null;
  }
  ...
  let serverLogger = function (buffer: any) {
