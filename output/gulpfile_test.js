...
import SC from "gulp-sourcemaps";
import OS from "os";

// === CONFIGURABLE VARIABLES

const bpfoldername = "tester";
...
const mcdir = useMinecraftDedicatedServer ? dedicatedServerPath : os.homedir() + (useMinecraftPreview ? "/AppData/Local/Packages/Microsoft.MinecraftWindowsBeta_8wekyb3d8bbwe/LocalState/games/com.mojang/" : "/AppData/Local/Packages/Microsoft.MinecraftUWP_8wekyb3d8bbwe/LocalState/games/com.mojang/");
...
function compile_scripts() {
  return gulp.src("scripts/**/*.ts").pipe(sourcemaps.init()).pipe(ts({
    module: "es2020",
...
function deploy_localmc_behavior_packs() {
  ...
  return gulp.src(["build/behavior_packs/" + bpfoldername + "/**/*"]).pipe(gulp.dest(mcdir + "development_behavior_packs/" + bpfoldername));
}
function deploy_localmc_resource_packs() {
  return gulp.src(["build/resource_packs/" + bpfoldername + "/**/*"]).pipe(gulp.dest(mcdir + "development_resource_packs/" + bpfoldername));
}
...
function backup_dev_world() {
  ...
  return gulp.src([getTargetWorldPath() + "/**/*"]).pipe(gulp.dest(getDevWorldBackupPath() + "/worlds/" + activeWorldFolderName));
}
...
function backup_localmc_world() {
  ...
  return gulp.src([getTargetWorldPath() + "/**/*"]).pipe(gulp.dest(getTargetWorldBackupPath() + "/" + activeWorldFolderName));
}
...
function startServer(callbackFunction) {
  ...
  let serverLogger = function (buffer) {
    ...
    if (incomingBuffer.endsWith("\n")) {
      (logBuffer + incomingBuffer).split(/\xn/).forEach(function (message) {
        if (message) {
